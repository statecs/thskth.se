...
export class ImageSliderSecondaryComponent implements OnInit {
  ...
  showAssociationInPopup(item: Association): void {
    this.router.navigate([this.lang + "/list/" + item.slug]);
  }
      ...
      if (self.slider) {
        ...
        const timmer2 = setInterval(function() {
          if (self.slider.nativeElement.getElementsByClassName("item")[0]) {
            ...
            const items = self.slider.nativeElement.getElementsByClassName(
              "item"
            );
            const wrapper_width: number =
              items.length * items[0].clientWidth + items.length * 10;
            const timmer3 = setInterval(function() {
              if (
                self.slider.nativeElement.getElementsByClassName(
                  "items-wrapper"
                )[0]
              ) {
                ...
                self.slider.nativeElement.getElementsByClassName(
                  "items-wrapper"
                )[0].style.width = wrapper_width + "px";
              }
            }, 100);
          }
        }, 100);
        self.touchStartListener = self.renderer.listen(
          self.slider.nativeElement,
          "touchstart",
          event => {
            self.showScroll();
          }
