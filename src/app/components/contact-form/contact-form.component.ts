...
export class ContactFormComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subjects = [
      ...
      {
        email: "karx@ths.kth.se",
        name: "Membership questions"
      },
      {
        email: "studiesocialt@ths.kth.se",
        name: "Student welfare"
      },
      {
        email: "utbildning@ths.kth.se",
        name: "Education"
      },
      {
        email: "naringsliv@ths.kth.se",
        name: "Business relations"
      },
      {
        email: "lokalbokning@ths.kth.se",
      ...
      {
        email: "restaurang@ths.kth.se",
        name: "Nymble Restaurant"
      },
      {
        email: "fastighet@ths.kth.se",
        name: "Report a fault in Nymble"
      },
      {
        email: "naringsliv@ths.kth.se",
        name: "Advertising"
      ...
      {
        email: "webmaster@ths.kth.se",
        name: "Webmaster"
      },
      {
        email: "kommunikation@ths.kth.se",
        name: "Övrigt/Other"
      }
    ];
    this.error = null;
  ...
  submitMessage(): void {
    ...
    this.success = null;
    if (
      this.contactForm.value.full_name === "" ||
      this.contactForm.value.message === "" ||
      this.contactForm.value.subject_to === "" ||
      this.contactForm.value.subject_to === "---"
    ) {
      if (this.lang === "en") {
        this.error = "Please fill in the required fields (*)!";
      } else {
        this.error = "Vänligen fyll i de obligatoriska fälten (*)!";
      }
    } else if (
      ...
      if (this.lang === "en") {
        this.error = "Please enter a correct email, this email is not valid.";
      } else {
        this.error = "Vänligen ange korrekt e-post.";
      }
    } else if (this.captcha.getResponse() === "") {
      if (this.lang === "en") {
        this.error = "Please resolve the reCAPTCHA and submit!";
      } else {
        this.error = "Vänligen fyll i reCAPTCHA och testa igen!";
      }
    } else {
      const post_data = {
      ...
      this.submitSubscription = this.contactFormService
        .submitMessage(post_data)
        .subscribe(
          res => {
            if (this.lang === "en") {
              ...
              this.success =
                "Mail skickat! Vi kommer att kontakta dig inom kort.";
            }
          },
          err => {
            if (this.lang === "en") {
