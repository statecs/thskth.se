...
export class MenusService extends WordpressBaseDataService<MenuItem> {
  ...
  get_secondarySubMenu(
    subMenu_slug: string,
    secondary_subMenu_slug: string,
    lang: string
  ): Observable<MenuItem[]> {
    return this.getTopLevel_mainMenu(lang).map(res => {
      return MenuItem.convertToSecondarySubMenu(
        res,
        subMenu_slug,
        secondary_subMenu_slug
      );
    });
  }
  ...
  getTopLevel_mainMenu(lang: string): Observable<MenuItem[]> {
    ...
    if (this.menus_meta) {
      ...
      return (
        this.getData(null, "order=desc&lang=" + lang)
