...
export class PostsService extends WordpressBaseDataService<Post> {
  ...
  getPosts(amount, lang: string): Observable<Post[]> {
    ...
    return (
      this.getData(null, "_embed&per_page=" + amount + "&lang=" + this.language)
        ...
        .map((res: any) => {
          return Post.convertToPostType(res);
        })
    );
  }

  getPostsBySinceDateTime(
    amount,
    lang: string,
  ...
  getOffers(amount, lang: string): Observable<Post[]> {
    ...
    return (
      this.getDataById(
        "_embed&sticky=true&per_page=" + amount + "&lang=" + this.language
      )
        // Cast response data to FAQ Category type
        .map((res: any) => {
          return Post.convertToPostType(res);
  ...
  getPostBySlug(slug, lang: string): Observable<Post> {
    ...
    return (
      this.getDataBySlug("_embed&slug=" + slug + "&lang=" + this.language)
        // Cast response data to FAQ Category type
        .map((res: any) => {
          return Post.convertToPostType(res)[0];
        })
    );
  }
  ...
  searchPosts(
    ...
    return (
      this.searchData(
        ...
        // Cast response data to FAQ Category type
        .map((res: any) => {
          return SearchResult.convertPostsToSearchResultType(res);
        })
    );
  }
}
