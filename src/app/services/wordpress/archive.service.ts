...
export class ArchiveService extends WordpressBaseDataService<Archive> {
  ...
  searchDocuments(searchParams: SearchParams, lang: string): Observable<Archive[]> {
    ...
    return this.searchData('_embed&search=' + searchParams.searchTerm + params + '&after=' + searchParams.start_date + 'T00:00:00' + '&before=' + searchParams.end_date + 'T23:59:00' + '&lang=' + this.language)
        // Cast response data to FAQ Category type
        .map((res: any) => { return this.castPostsTo_SearchResultType(res); });
  }
